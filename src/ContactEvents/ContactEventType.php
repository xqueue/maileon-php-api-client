...
use Maileon\ContactEvents\DataType;
...
class ContactEventType extends AbstractXMLWrapper
{
    ...
    public function fromXML($xmlElement)
    {
        ...
        if (isset($xmlElement->active)) {
            $this->active = $xmlElement->active;
        }
        if (isset($xmlElement->anonymizable)) {
            $this->anonymizable = $xmlElement->anonymizable;
        }
        ...
        if (isset($xmlElement->attributes)) {
            ...
            foreach ($xmlElement->attributes->children() as $xmlAttribute) {
                ...
                if (isset($xmlAttribute->name)) {
                    $attribute['name'] = trim($xmlAttribute->name);
                }
                if (isset($xmlAttribute->datatype)) {
                    $attribute['datatype'] = DataType::getDataType($xmlAttribute->datatype);
                }
                if (isset($xmlAttribute->description)) {
                    $attribute['description'] = trim($xmlAttribute->description);
                }
                if (isset($xmlAttribute->required)) {
                    $attribute['required'] = $xmlAttribute->required;
                }
    ...
    public function toString()
    {
        ...
        if (isset($this->attributes)) {
            foreach ($this->attributes as $index => $value) {
                $attributes .= "attribute (name=" . $value['name'] . ", datatype=" .
                    $value['datatype']->getValue() . ", description=" . $value['description'] .
        ...
        return "ContactEventType [id=" . $this->id . ", name=" . $this->name .
            ", active=" . $this->active . ", anonymizable=" . $this->anonymizable .
            ", description=" . $this->description . ", created=" . $this->created .
            ", updated=" . $this->updated . ", attributes=" . $attributes . "]";
    }
    ...
    public function toXML()
    {
        ...
        if (isset($this->active)) {
            $xml->addChild("active", $this->active);
        }
        if (isset($this->anonymizable)) {
            $xml->addChild("anonymizable", ($this->anonymizable == true) ? "true" : "false");
        }
